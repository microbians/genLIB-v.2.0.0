...
loadLayer.prototype.createIFRAME=function(){
	if (!(is.ie50&&is.mac)) this.setHTML('<IFRAME ID="'+this.iframename+'" src="about:blank" STYLE="position:absolute;visibility:hidden;width:0px;height:0px;"></IFRAME>',is.ie?true:false);
	else 	 		this.setHTML('<IFRAME ID="'+this.iframename+'" src="about:blank" STYLE="position:absolute;left:-2000px;top:-2000px;width:200px;height:200px;"></IFRAME>',true);
}
...
loadLayer.correctHTML=function(url,html) {
	...
	outCode = outCode.replace( / src=([\'|\"]*)([^\'|\"|>| ]+)([\'|\"]*)/ig , ' src=\"$2\"');
	outCode = outCode.replace( /src\=([\"|\'])([^\/|#])/ig , 'src=$1'+url+'$2');
	outCode = outCode.replace( /src\=\"([^f]+)file:/ig , 'src=\"file:');
	outCode = outCode.replace( /src\=\"([^h]+)http:/ig , 'src=\"http:');

	outCode = outCode.replace( /href\=([\"|\'])([^\/|http\:|#|javascript\:|mailto\:])/ig , 'href=$1'+url+'$2');

	if (is.ie) outCode = outCode.replace( /\<IMG /ig , '<IMG onload="this.ready=true" onerror="this.ready=true" ');
